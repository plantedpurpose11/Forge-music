...
class DiscordMusicBot extends Client {
  ...
  build() {
    ...
    // Initialize manager AFTER client is ready to avoid race condition
    this.once("ready", () => {
      this.warn("Client ready, initializing Lavalink manager...");
      this.initManager();
    });
  }

  /**
   * Initialize the Lavalink manager (called after client is ready)
   */
  initManager() {
    let client = this;
    ...
    this.manager = new Manager({
      ...
      .on("nodeError", (node, err) => {
        ...
        console.error("Node Error details:", err);
      })
